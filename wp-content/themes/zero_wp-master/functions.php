...
function theme_styles_and_scripts() {
	...
	wp_enqueue_style('swiper-css', $css_url . 'swiper-bundle.min.css', array(), $ver);

	wp_enqueue_script('swiper-lib', $js_url . 'swiper-bundle.min.js', array(), $ver, array(
		'in_footer' => true,
		'strategy' => 'defer',
	));

	// Подключаем инициализацию Swiper с зависимостью от библиотеки
	wp_enqueue_script('swiper-init', $js_url . 'swiper.js', array('swiper-lib'), $ver, array(
		'in_footer' => true,
		'strategy' => 'defer',
	));

	$floors = array('about', 'customers', 'testimonials', 'globalization', 'leadership', 'services');

	foreach ($floors as $floor) {
		wp_enqueue_style('css-' . $floor, get_template_directory_uri() . '/floors/' . $floor . '/' . $floor . '.css', array(), $ver); 
	}

	wp_enqueue_style('fonts-gstatic', 'https://fonts.gstatic.com');
	wp_enqueue_style('fonts', 'https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300..800;1,300..800&display=swap" rel="stylesheet'); 
}
